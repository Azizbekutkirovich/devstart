...
function checkingTest() {
  ...
  $.ajax({
    ...
    dataType: 'json',
    contentType: 'application/json',
    data: JSON.stringify({ selected: selectedAnswers }),
    success(response) {
      hideLoader(loader);
      if (!response.success) {
        addBotMessage('❌ Xatolik: ' + response.message);
...
function _applyTestResults(results, quizDiv) {
  quizDiv.querySelectorAll('.question-block').forEach((block, i) => {
    const res = results[i];
